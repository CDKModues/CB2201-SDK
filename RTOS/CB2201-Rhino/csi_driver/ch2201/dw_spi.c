...
void dw_spi_dma_event_cb(int32_t ch, dma_event_e event)
{
    ...
    if (event == DMA_EVENT_TRANSFER_ERROR) {           /* DMA transfer ERROR */
        csi_vic_enable_irq(spi_priv->idx);
...
static int32_t dw_spi_send_dma(dw_spi_priv_t *spi_priv, const void *data, uint32_t num)
{
    ...
    int32_t ret = csi_dma_config_channel(spi_priv->dma_tx_ch, &config, dw_spi_dma_event_cb);
...
static int32_t dw_spi_send_dma(dw_spi_priv_t *spi_priv, const void *data, uint32_t num)
{
    ...
    spi_priv->clk_num       = num;
    spi_priv->send_num       = num;
    ret = csi_dma_config_channel(spi_priv->dma_tx_ch, &config, dw_spi_dma_event_cb);
...
static int32_t dw_spi_receive_dma(dw_spi_priv_t *spi_priv, void *data, uint32_t num)
{
    ...
    ret = csi_dma_config_channel(spi_priv->dma_rx_ch, &config, dw_spi_dma_event_cb);
...
static int32_t dw_spi_transfer_dma(dw_spi_priv_t *spi_priv, const void *data_out, void *data_in, uint32_t num_out, uint32_t num_in)
{
    ...
    ret = csi_dma_config_channel(spi_priv->dma_tx_ch, &tx_config, dw_spi_dma_event_cb);
    ...
    ret = csi_dma_config_channel(spi_priv->dma_rx_ch, &rx_config, dw_spi_dma_event_cb);
