...
static uint64_t ck_rtc_readtime(ck_rtc_reg_t *addr)
{
    ...
    time = (time - load) * 16384 / (drv_get_sys_freq() / 1000);
#elif CONFIG_CHIP_DANICA
    uint32_t clockdiv = addr->RTC_DIV;
    clockdiv = (clockdiv + 1) * 2;
    /* danica fpga count clock source is 48M */
    time = (time - load) / ((drv_get_sys_freq() * 2 / clockdiv) / 1000);
#else
    uint32_t clockdiv = addr->RTC_DIV;
    clockdiv = (clockdiv + 1) * 2;
    time = (time - load) / ((drv_get_sys_freq() / clockdiv) / 1000);
#endif
    return time;
}
...
static int ck_rtc_setmarchtime(ck_rtc_reg_t *addr, int64_t settime)
{
    ...
    clockdiv = (clockdiv + 1) * 2;
    /* danica fpga count clock source is 48M */
    time  = time * ((drv_get_sys_freq() * 2 / clockdiv) / 1000);
#else
    uint32_t clockdiv = addr->RTC_DIV;
    clockdiv = (clockdiv + 1) * 2;
    time  = time * ((drv_get_sys_freq() / clockdiv) / 1000);
#endif
