...
extern int32_t __attribute__((weak)) target_get_dmac(uint32_t idx, uint32_t *base, uint32_t *irq);
...
void dw_dmac_irqhandler(int32_t idx)
{
        ...
        for (count = 0; count < CONFIG_DMA_CHANNEL_NUM / 2; count++) {
            if ((temp & (1U << count)) != 0) {
                ...
                if (dma_priv->cb_event) {
                    dma_priv->cb_event(count + idx * 2, DMA_EVENT_TRANSFER_DONE);
                }
    ...
    if (temp) {
        ...
        if (dma_priv->cb_event) {
            dma_priv->cb_event(count + idx * 2, DMA_EVENT_TRANSFER_ERROR);
        }
    }
}
...
int32_t csi_dma_alloc_channel(void)
{
    ...
    target_get_dmac(ch_num, &base, &irq);
